"""
verify_claims.py — Verso-inspired claim verifier for markdown documents.

Embeds typed claims as HTML comments next to prose:

    parse_claims takes a `text` argument.
    <!-- claim: signature target=verify_claims.parse_claims has-params=text -->
...
Run:
    python3 verify_claims.py path/to/file.md
    python3 verify_claims.py --json path/to/file.md
...
def resolve_signature(args: dict) -> Result:
    ...
    # can't trigger arbitrary imports. Override via VERIFY_CLAIMS_IMPORT_ALLOW
    # (comma-separated prefixes).
    allow = os.environ.get('VERIFY_CLAIMS_IMPORT_ALLOW', 'muninn_utils,scripts,verify_claims')
    ...
    if top not in prefixes:
        return Result('error',
                      detail=f'import of {mod_path!r} blocked; '
                             ...
                             f'(set VERIFY_CLAIMS_IMPORT_ALLOW to extend)')
    try:
...
def watch(path: str, interval: float = 0.5) -> int:
    ...
    print(f'verifying-claims --watch {path}  (re-runs on save)')
    last_rc = run_once()
...
def main():
    ...
    if '--watch' in args:
        ...
        args.remove('--watch')
    if len(args) != 1:
        print('usage: verify_claims.py [--json] [--watch] path/to/file.md', file=sys.stderr)
        return 2
    if watch_mode:
        return watch(args[0])
