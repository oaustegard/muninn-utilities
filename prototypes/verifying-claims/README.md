...
"`verify_claims.py demo.md` exits 0"). The claim and the thing it describes are
...
python3 verify_claims.py --watch demo.md   # re-verify on every save (inner-loop aid)
```

The demo is self-referential: `demo.md` is the README *and* the test suite
for `verify_claims.py` — its claims verify `verify_claims.py` itself. One claim deliberately
...
- `importlib.import_module` in `resolve_signature` executes target module
  ...
  module prefixes (`muninn_utils`, `scripts`, `verifying-claims` by default; override
  via `VERIFY_CLAIMS_IMPORT_ALLOW`). A crafted claim can't `import os` and run it.
...
verifying-claims to an event with its own enforcement, ranked by how hard it is to skip:

1. **Required CI check on PRs** — strongest. A workflow runs `verifying-claims spec.md`;
   branch protection makes it a required status. You cannot merge red. The
   enforcement is structural, not disciplinary.
2. **Claims as part of the pytest suite** — a `test_verifying-claims_claims` runs
   `verifying-claims spec.md` and asserts exit 0, so verifying-claims runs whenever tests run and
   inherits the same green-bar gate. Best fit when verifying-claims wraps a TDD loop.
...
- A GitHub Action + `test_verifying-claims_claims` so claims gate merges (see "What forces a run")
...
- A `verifying-claims` directive that embeds a live value inline at render time
